...
def merge_duplicate_genomic_locations(df: pl.DataFrame) -> pl.DataFrame:
    """
    Merge duplicate genomic locations (same chrom:chromStart:strand) using weighted average frequency.
    
    Formula: mod_freq(genomic_site) = (mod_freq_1 * coverage_1 + ... + mod_freq_n * coverage_n) / total_coverage
    """
    # Identify coverage and frequency columns
    coverage_col = "column_10" if "column_10" in df.columns else None
    frequency_col = "column_11" if "column_11" in df.columns else None
    
    if coverage_col is None or frequency_col is None:
        logger.warning("Coverage or frequency columns not found; skipping deduplication")
        return df
    
    # Group by chrom, chromStart, and strand
    grouped = df.group_by(["chrom", "chromStart", "column_6"]).agg([
        pl.col(coverage_col).sum().alias("total_coverage"),
        (pl.col(frequency_col) * pl.col(coverage_col)).sum().alias("weighted_freq_sum"),
        pl.first("column_3"),
        pl.first("column_4"),
        pl.first("column_5"),
        pl.first("column_7"),
    ...
    deduped = grouped.with_columns(
        (pl.col("weighted_freq_sum") / pl.col("total_coverage")).alias("weighted_frequency")
    ).select([
        pl.col("chrom"),
        pl.col("chromStart"),
        pl.col("column_3"),
        ...
        pl.col("column_8"),
        pl.col("column_9"),
        pl.col("total_coverage").alias(coverage_col),
        pl.col("weighted_frequency").alias(frequency_col),
    ])
    
    return deduped


def write_bed_rmod_with_header(df: pl.DataFrame, output_path: str, header_lines: list[str], plain_header_line: str | None) -> None:
...
def main() -> None:
    ...
    has_column_6 = "column_6" in bedrmod.columns
    ...
    if args.output:
        ...
        if has_column_6:
            out_columns.append(pl.Series(name="column_6", values=output_strand, strict=False))
        else:
            out_columns.append(pl.Series(name="column_6", values=output_strand, strict=False))
        ...
        # Merge duplicate genomic locations with weighted average frequency
        logger.info("Merging duplicate genomic locations...")
        out_df = merge_duplicate_genomic_locations(out_df)
        logger.info("After deduplication: %d unique genomic sites", out_df.shape[0])
